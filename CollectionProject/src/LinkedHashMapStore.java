...
/**
	*@author AJAY E.S 
	*Page for LinkedHashmap operations
	**/
...
import java.util.logging.Logger;
	...
	public class LinkedHashMapStore<T>implements  CollectionRepository{
		
		Map<Integer,T> store=new LinkedHashMap<Integer,T>(); 
		Class <T>type;
		public LinkedHashMapStore(Class<T>type) {
			this.type=type;
		}
		private static final Logger log=Logger.getLogger(LinkedHashMapStore.class.getName());
		public LinkedHashMapStore(){
			log.setLevel(Level.WARNING);
		}
			public void display(int ty) {
				log.info("Started performing Operations in LinkedHashMap");
				System.out.println("Intially LinkedHashMap:");
				read();
				...
				read();
				log.info("Ended performing Operations in LinkedHashMap");
			}
			public  void creater(Object data) {
				int i=store.size();
				store.put(i, (T) data);
				log.info("Sucessfully added value to LinkedHashMap!");
			}
			public void remover() {
