	/**
	*@author AJAY E.S 
	*Page for LinkedList operations
	**/
	...
	public class LinkedListStore<T> implements CollectionRepository{
	List<T> store=new LinkedList<T>(); 
	Class <T>type;
	public LinkedListStore(Class<T>type) {
		this.type=type;
	}
	private static final Logger log=Logger.getLogger(LinkedListStore.class.getName());
	public LinkedListStore(){
		log.setLevel(Level.WARNING);
	}
	//Method to start or display the details	
	public void display(int ty) {
		log.info("Started performing Operations in LinkedList");
			System.out.println("Intially LinkedList:");
			read();
			...
			read();
			log.info("Ended performing Operations in LinkedList");
		}
		public  void creater(Object data) {
			store.add((T) data);
			log.info("Sucessfully added value to LinkedList!");
		}

		public void read() {
		   if(store.size()==0) {
				System.out.println("LinkedList is empty!!");
				log.warning("No data has been inserted!");
			}
			else {			
