...
import java.util.logging.Logger;
	...
	public class HashMapStore<T>implements  CollectionRepository{
		
		Map<Integer,T> store=new HashMap<Integer,T>(); 
		Class <T>type;
		public HashMapStore(Class<T>type) {
			this.type=type;
		}
		private static final Logger log=Logger.getLogger(HashMapStore.class.getName());
		public HashMapStore(){
			log.setLevel(Level.WARNING);
		}
			public void display(int ty) {
				log.info("Started performing Operations in HashMap");
				System.out.println("Intially HashMap:");
				read();
				...
				read();
				log.info("Ended performing Operations in HashMap");
			}
			public  void creater(Object data) {
				int i=store.size();
				store.put(i, (T) data);
				log.info("Sucessfully added value to HashMap!");
			}
			public void remover() {
