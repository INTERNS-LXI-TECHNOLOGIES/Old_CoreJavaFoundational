...
public class Forest{
	...
	public void canFight(Animal c){
		for(int i=0;i<7;i++)
		{
			for(int j=0;j<7;j++)
			{    
				if((check(c))&&(a[i][j]!=c)&&(a[i][j]!=null)&&(Math.abs(c.getXloc()-i)<=c.getReach())&&(c instanceof Carnivorus)&&(Math.abs(c.getYloc()-j)<=c.getReach())){
					
					if((c.getStrength()>a[i][j].getStrength())&&(a[i][j] instanceof Herbivorus)&&(a[i][j].getLuck()<3)){
						if(c instanceof Lion){
						((Lion)c).fight(a[i][j],c);
						((Lion)c).eats(a[i][j],c);//casting
					    a[i][j]=null;}
						if(c instanceof Tiger){
						((Tiger)c).fight(a[i][j],c);
						((Tiger)c).eats(a[i][j],c);//casting
					    a[i][j]=null;}}
				    else if(a[i][j] instanceof Herbivorus &&(a[i][j].getLuck()>3))
